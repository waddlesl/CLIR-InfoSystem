...
namespace CLIR_InfoSystem.Controllers
{
    public class ReportController : Controller
    {
        ...
        public async Task<IActionResult> DownloadReport()
        {
            try
            {
                int totalPatrons = 2094;
                string topBook = "Clean Code";
                var pdfBuffer = await ExportPdfAsync(totalPatrons, topBook);

                return File(pdfBuffer, "application/pdf", "UsageReport.pdf");
            }
            catch (Exception ex)
            {
        ...
        private async Task<byte[]> ExportPdfAsync(int totalPatrons, string topBook)
        {
        ...
        private string GetHtmlContent()
        {
            ...
            var PatronSHS = patrons.Count(p => p.Department == "SHS");
            var PatronCollege = patrons.Count(p => p.Department != "SHS");
            ...
            var GrammarlyCountForCollege = requests.Count(gat => gat.Patron != null && gat.Patron.Department != "SHS" && gat.ServiceType == "Grammarly");
            var GrammarlyCountForSHS = requests.Count(gat => gat.Patron != null && gat.Patron.Department == "SHS" && gat.ServiceType == "Grammarly");
            var TurnitinCountForCollege = requests.Count(gat => gat.Patron != null && gat.Patron.Department != "SHS" && gat.ServiceType == "Grammarly");
            var TurnitinCountForSHS = requests.Count(gat => gat.Patron != null && gat.Patron.Department == "SHS" && gat.ServiceType == "Grammarly");

            //for book a librarian
            var bookings = _context.BookALibrarians.Include(b => b.Patron).ToList();

            var LBookingCountForCollege = bookings.Count(sb => sb.Patron != null && sb.Patron.Department != "SHS");
            var LBookingCountForSHS = bookings.Count(sb => sb.Patron != null && sb.Patron.Department == "SHS");
            ...
            var BookBorrowCountForCollege = borrowings.Count(sb => sb.Patron != null && sb.Patron.Department != "SHS");
            var BookBorrowCountForSHS = borrowings.Count(sb => sb.Patron != null && sb.Patron.Department == "SHS");
            ...
            // for ODDS
            var odds = _context.Odds.Include(r => r.Patron).ToList();

            var ODDSCountForCollege = odds.Count(sb => sb.Patron != null && sb.Patron.Department != "SHS");
            var ODDSCountForSHS = odds.Count(sb => sb.Patron != null && sb.Patron.Department == "SHS");
